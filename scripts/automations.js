...
export function makeActionApi(api) {
  ...
  return {
    ram: () => runRam(api),
	fragmentSignal: () => runFragmentSignal(api)
  };
}

async function runRam(api) {
  const attackerToken = getActingMechToken();
...
function getActingMechToken() {
  const controlled = canvas.tokens.controlled;

  if (controlled.length !== 1) {
    ui.notifications.warn("Select exactly one acting mech token.");
    return null;
  }

  const selectedToken = controlled[0];
  ...
  // Normal case: the player selected their mech.
  if (selectedActor.type === "mech") {
    return selectedToken;
  }

  // Optional convenience: if the player selected their pilot,
  // try to resolve the pilot's active mech.
  if (selectedActor.type === "pilot") {
    const activeMech =
      selectedActor.system?.active_mech?.value ??
      selectedActor.system?.active_mech?.actor ??
      null;

    const activeMechToken = findTokenForActor(activeMech);

    if (activeMechToken) {
      return activeMechToken;
    }

    ui.notifications.warn("Selected pilot has no active mech token on the scene.");
    return null;
  }

  ui.notifications.warn("Selected token must be a mech, or a pilot with an active mech.");
  return null;
}
...
async function runFragmentSignal(api) {
  const attackerToken = getActingMechToken();
  if (!attackerToken) return;
  ...
  if (!TechAttackFlow) {
    ui.notifications.error("Could not find Lancer TechAttackFlow.");
    return;
  }

  const flow = new TechAttackFlow(attackerToken.actor, {
    title: "Fragment Signal",
    effect: "On hit: the target becomes Impaired and Slowed until the end of its next turn, and takes 2 Heat."
  });
  ...
  if (!success) {
    ui.notifications.info("Fragment Signal was cancelled.");
    return;
  }
  ...
  if (!result) {
    ui.notifications.warn(
      "Fragment Signal completed, but no hit result was found. Make sure Lancer attack automation is enabled and that exactly one target was selected."
    );
    return;
  }
  ...
  if (!result.hit) {
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ token: attackerToken }),
      content: `<p><strong>Fragment Signal:</strong> ${attackerToken.name} missed ${targetToken.name}. No conditions or Heat were applied.</p>`
    });

    return;
  }

  await api.applyTimedStatusByPlayer("impaired", [targetToken], { turns: 1 });
  await api.applyTimedStatusByPlayer("slow", [targetToken], { turns: 1 });
  await api.applyHeatByPlayer(2, [targetToken]);

  await ChatMessage.create({
    speaker: ChatMessage.getSpeaker({ token: attackerToken }),
    content: `<p><strong>Fragment Signal:</strong> ${attackerToken.name} hit ${targetToken.name}. Applied <strong>Impaired</strong>, <strong>Slowed</strong>, and <strong>2 Heat</strong>.</p>`
  });
}